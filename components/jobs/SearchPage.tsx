// components/jobs/SearchPage.tsx (fixed)
'use client';
...
import { Job, JobFilters, JobSearchResponse, AISearchResponse } from '@/lib/types/job.types';
...
interface SearchPageProps {
    initialData?: JobSearchResponse | null;
}
...
export default function SearchPage({ initialData }: SearchPageProps) {
    ...
    // Determine if we should use initial data - only when no search params are present
    ...
    const {
        ...
        enabled: searchMode === 'standard' && !shouldUseInitialData,
    ...
    const handleFilterChange = (key: keyof JobFilters, value: any) => {
        ...
        if (newFilters.minSalary) params.set('minSalary', String(newFilters.minSalary));
        if (newFilters.maxSalary) params.set('maxSalary', String(newFilters.maxSalary));
    ...
    const getDisplayData = (): JobSearchResponse | undefined => {
    ...
    const isLoading = searchMode === 'standard'
        ? standardLoading && !shouldUseInitialData
        : aiLoading;
    ...
    return (
        <>
            ...
            <Tabs
                value={searchMode}
                onValueChange={(value) => {
                    setSearchMode(value as 'standard' | 'ai');
                }}
                className="mb-8"
            >
